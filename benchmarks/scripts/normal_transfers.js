...
const pairs = wallets.normal_pairs;
...
export default function () {
  // Each VU maps to its own dedicated disjoint pair to eliminate cross-request row locks
  const pairIdx = (__VU - 1) % pairs.length;
  const pair = pairs[pairIdx];
  const fromWallet = pair[0];
  const toWallet = pair[1];
  ...
  const idempotencyKey = `norm-${__VU}-${__ITER}-${Date.now()}-${crypto.randomUUID()}`;
